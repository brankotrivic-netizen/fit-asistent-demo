...
const CATEGORY_VALUES = new Set([CATEGORIES.ORDER, CATEGORIES.INSTALLATION, CATEGORIES.SERVICE]);
...
export function evaluateEmailCategory(input = {}) {
  const category = text(input.category, 80).toLowerCase();
  const hasCategoryConfidence = input.category_confidence !== undefined && input.category_confidence !== null && String(input.category_confidence).trim() !== "";
  const categoryConfidence = confidenceNumber(input.category_confidence, 0);
  const valid = CATEGORY_VALUES.has(category);
  const confident = hasCategoryConfidence && categoryConfidence >= 0.85;
  return {
    category: valid && confident ? category : CATEGORIES.MANUAL_REVIEW,
    category_confidence: hasCategoryConfidence ? categoryConfidence : 0,
    category_review_required: !valid || !confident,
  };
}

function result(decision, safeToAutoSend, reason, confidence, missingData, draftReply, categoryOutput) {
  return { decision, safe_to_auto_send: safeToAutoSend, decision_reason: reason, confidence, missing_data: missingData, draft_reply: draftReply, ...categoryOutput };
}
...
export function evaluateEmailDecision(input = {}) {
  ...
  const categoryOutput = evaluateEmailCategory(input);

  if (categoryOutput.category_review_required && input.enforce_category !== false) {
    return result(DECISIONS.MANUAL, false, "Kategorija ni dovolj zanesljiva, zato je sporočilo usmerjeno v Ročni pregled.", confidence, missingData, draftReply, categoryOutput);
  }

  if (MANUAL_PATTERNS.some((pattern) => pattern.test(combined))) {
    return result(DECISIONS.MANUAL, false, "Zaznana je reklamacija, varnostni incident, nujna ali pravno občutljiva zadeva.", confidence, missingData, draftReply, categoryOutput);
  }
  if (confidence < 0.9) {
    return result(DECISIONS.MANUAL, false, "AI confidence je nižji od 0,90, zato je potrebna ročna obdelava.", confidence, missingData, draftReply, categoryOutput);
  }
  if (proposedDecision === DECISIONS.MANUAL) {
    return result(DECISIONS.MANUAL, false, text(input.decision_reason, 1000) || "AI je zahteval ročno obdelavo.", confidence, missingData, draftReply, categoryOutput);
  }
  if (autoReplyCount >= 1) {
    return result(DECISIONS.REVIEW, false, "V tem e-mail threadu je bil samodejni clarification odgovor že poslan.", confidence, missingData, draftReply, categoryOutput);
  }
  if (REVIEW_PATTERNS.some((pattern) => pattern.test(combined))) {
    return result(DECISIONS.REVIEW, false, "Vsebina zahteva odločitev o ceni, terminu, izvedbi ali tehnični rešitvi.", confidence, missingData, draftReply, categoryOutput);
  }
  if (proposedDecision === DECISIONS.REVIEW) {
    return result(DECISIONS.REVIEW, false, text(input.decision_reason, 1000) || "AI je pripravil osnutek za človeško potrditev.", confidence, missingData, draftReply, categoryOutput);
  }
  if (!allMissingDataIsSafe(missingData)) {
    return result(DECISIONS.REVIEW, false, "Manjkajoči podatki niso omejeni na varna, nevtralna pojasnila.", confidence, missingData, draftReply, categoryOutput);
  }
  if (!isSafeClarificationDraft(draftReply)) {
    return result(DECISIONS.REVIEW, false, "Osnutek ni prestal varnostnega pregleda za samodejno pošiljanje.", confidence, missingData, draftReply, categoryOutput);
  }
  if (proposedDecision === DECISIONS.AUTO && input.safe_to_auto_send !== true) {
    return result(DECISIONS.REVIEW, false, "AI ni izrecno potrdil safe_to_auto_send, zato je potrebna človeška potrditev.", confidence, missingData, draftReply, categoryOutput);
  }
  if (proposedDecision && proposedDecision !== DECISIONS.AUTO) {
    return result(DECISIONS.REVIEW, false, "Če obstaja dvom, je potrebna človeška potrditev.", confidence, missingData, draftReply, categoryOutput);
  }
  return result(DECISIONS.AUTO, true, text(input.decision_reason, 1000) || "AI sprašuje samo po manjkajočih nevtralnih podatkih; tveganih obljub ali odločitev ni.", confidence, missingData, draftReply, categoryOutput);
}
